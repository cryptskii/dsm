...
| **§4.5.7 single-draw admission brittleness** (Phase 9) | `classify_m_sample_*` unit tests + observed Galaxy A54 variance band (h_hat=0.41/0.77/0.92/0.94 across runs) | **Replaced** by median-of-M admission rule with per-sample hard floors.  M=3 admission samples × K=21 trials/sample. First-genesis cost rises ~5min → ~15min (one-time per device). Revision 4/5/6 enrollment files invalidate one-time on first launch under v7. Spec H_HAT_MIN=0.45 remains the median floor; H_HAT_HARD_REJECT=0.35 catches any catastrophic per-sample failure. The MARGINAL band [WARN=0.40, MIN=0.45) maps to ResonantStatus::Resonant → AccessLevel::ReadOnly — degraded but usable, no longer alternating verdicts across runs. |
